...
#The UI part of the code defines how your app will look and what elements (like buttons, inputs, and outputs). ui <- fluidPage(
...
observeEvent(input$calculate_stock, { req(input$Cstock, #Cs=Concentration of the stock solution input$Cdesired, #Cx=Desired final concentration input$final_volume) #Vx=Final total volume
...
#Combines the UI and server components and starts the Shiny app. shinyApp(ui = ui, server = server)o
